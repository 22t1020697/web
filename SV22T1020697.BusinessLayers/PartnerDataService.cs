...
public static class PartnerDataService
{
    ...
    private static readonly EmployeeAccountRepository employeeAccountDB;
    ...
    public static async Task<bool> ChangeCustomerPasswordAsync(int id, string newPassword)
    {
        ...
        return await customerDB.ChangePasswordAsync(id, hashedPassword);
    }

    public static async Task<bool> ChangeCustomerPasswordAsync(int id, string oldPassword, string newPassword)
    {
        var customer = await customerDB.GetAsync(id);
        if (customer == null) return false;

        if (customer.PasswordHash != HashPassword(oldPassword)) return false;
    ...
    // Đổi mật khẩu theo Email (có kiểm tra mật khẩu cũ)
    public static async Task<bool> ChangeEmployeePasswordAsync(string email, string oldPassword, string newPassword)
    {
        ...
        return await employeeAccountDB.ChangePasswordAsync(email, oldHashed, newHashed);
    }
    // Đổi mật khẩu theo EmployeeID (Controller sẽ gọi vào đây)
    public static async Task<bool> ChangeEmployeePasswordAsync(int employeeID, string oldPassword, string newPassword)
    {
        ...
        var emp = await employeeDB.GetAsync(employeeID);
        if (emp == null) return false;

        return await ChangeEmployeePasswordAsync(emp.Email, oldPassword, newPassword);
    }

    public static async Task<bool> IsUsedCategoryAsync(int shipperID)
    {
        return await categoryDB.IsUsedAsync(shipperID);
    }
